...
test('client hosts cannot reach parked ordering APIs or kitchen; pitch host retains the demo',()=>{
 ...
 for(const host of ['copperac.com','www.copperac.com','copperac.vercel.app']){
  for(const route of ['/api/ordering/order','/api/ordering/attempt','/api/kitchen/orders','/api/kitchen/operation','/api/printer'])assert.equal(proxy(request(host,route)).status,404);
