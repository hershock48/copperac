...
function load(file,mocks={},env={},fetch){
 ...
 new vm.Script(code).runInNewContext({module,exports:module.exports,require:name=>{
  ...
  if(Object.hasOwn(mocks,name))return mocks[name];throw Error('Unexpected import '+name);
 },process:{env},Buffer,Date:Clock,URL,AbortSignal,fetch,console:{error(){}}});return module.exports;
}
...
test('login throttling has a bounded local window and requires persistence in production',async()=>{
 ...
 for(let i=0;i<5;i++)assert.equal(await dev.allowLogin(1000),true);
 assert.equal(await dev.allowLogin(1000),false);assert.equal(await dev.allowLogin(601001),true);
 await assert.rejects(load('lib/workroom/login-limit.ts',mocks,{NODE_ENV:'production'}).allowLogin());
});
test('all workroom mutation routes refuse volatile production saves before reading request bodies',async()=>{
 ...
 for(const [file,method] of [['events','PUT'],['events','DELETE'],['menu','PUT'],['events/contact','PUT'],['events/image','POST']]){
  const route=load(`app/api/workroom/${file}/route.ts`,{...next,'next/cache':{},'@/lib/workroom/auth':{isWorkroomAuthed:async()=>true},'@/lib/workroom/write-guard':guard,'@/lib/workroom/store':{},'@/lib/content':{},'@/lib/workroom/events-def':{},'@/lib/workroom/menu-def':{}});
  const response=await route[method]({json:async()=>{throw Error('Must reject before parsing');}});
  assert.equal(response.status,503,file);assert.match(response.body.error,/not been saved/);
 }
});
test('login rejects null input, throttled attempts and storage outages without issuing cookies',async()=>{
 for(const scenario of ['null','limited','storage']){
  let issued=0;
  const route=load('app/api/workroom/login/route.ts',{...next,'@/lib/workroom/auth':{workroomPasscode:()=>passcode,workroomSessionReady:()=>true,passcodeMatches:()=>false,setWorkroomCookie:async()=>{issued++;}},'@/lib/workroom/login-limit':{allowLogin:async()=>{if(scenario==='storage')throw Error('Offline');return scenario!=='limited';},clearLoginAttempts:async()=>{}}});
...
test('client hosts cannot reach parked ordering APIs or kitchen; pitch host retains the demo',()=>{
 ...
 for(const host of ['copperac.com','www.copperac.com','copperac.vercel.app']){
  for(const route of ['/api/ordering/order','/api/kitchen/orders','/api/printer'])assert.equal(proxy(request(host,route)).status,404);
